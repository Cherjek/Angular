...
import { CurrentDataService } from "../../../../services/currentdata.module/currentdata.service";
import { PageCurrentData } from "../../../../services/currentdata.module/Models/PageCurrentData";

import { DataResultSettingsService } from '../../../../services/datapresentation.module/DataResultSettings.service';

declare var $: any;

const keyStorageCategoryPosition = "U.OECurrentDataComponent.storageCategoryPosition";

const notFoundClass = 'data-not-found-2';
const filterAttribute = 'data-filter';

@Component({
    selector: 'oe-currentdata',
    templateUrl: './oe.currentdata.html',
    styleUrls: ['./oe.currentdata.css'],
    providers: [CurrentDataService]
})
export class OECurrentDataComponent implements OnInit, OnDestroy, AfterContentChecked {
    categoryId: number;
    ...
    private get storageCategoryPosition() {
        ...
        return storage != null ? Object.assign(new PageCurrentData(), JSON.parse(storage)) : null;
    }
    ...
    private searchByFilter() {
        if (this.searchFilter && this.searchFilter.trim().length) {
            $(`*[${filterAttribute}]`).each((index: number, el: HTMLElement) => {
                if ($(el).attr(`${filterAttribute}`).toLowerCase().indexOf(this.searchFilter.toLowerCase()) !== -1) {
    ...
    public print() {
        ...
        this.currentDataService.print(this.unitId, this.selectedPage.Id)
            .subscribe((data: any) => {
                    this.loadingContent = false;
                    ...
                    if ((<any>window).printableFrame) {
                        ...
                        iFrame.contentWindow.document.write(`${data}<script>window.print()</script>`);
    ...
    public export() {
        this.loadingContent = true;
        this.currentDataService.export(this.unitId, this.selectedPage.Id)
            .subscribe((data: any) => {
                    this.loadingContent = false;
