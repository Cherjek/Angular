...
import {
  ILogicDeviceCommandTypeParameterOption,
  LogicDeviceCommandTypeParameterOption
} from 'src/app/services/commands/Models/LogicDeviceCommandTypeParameterOption';
...
export class TypeLogicDevicesOptionsPropertyCardComponent
  ...
  constructor(
    private deviceTypeCommParamOptionService: LogicDeviceTypeCommandParameterOptionService,
    ...
    this.parameterId = this.activatedRoute.parent.snapshot.params.parameterId;
  ...
  private loadDevices() {
    ...
    this.sub$ = this.deviceTypeCommParamOptionService
      ...
      .subscribe(
        (data: ILogicDeviceCommandTypeParameterOption) => {
  ...
  private initProperties(device: ILogicDeviceCommandTypeParameterOption) {
    this.properties = [
      {
        Code: 'Text',
  ...
  public save(properties: IEntityViewProperty[], propControl: any) {
    this.errors = [];
    const device = new LogicDeviceCommandTypeParameterOption();
    if (!this.isNew) {
      device.Id = this.optionId as number;
    }
    device.IdLogicDeviceCommandTypeParameter = this.parameterId;
      ...
      .then(optionId => {
        ...
        if (this.isNew) {
          this.router.navigateByUrl(
            `commands-module/o/types-commands-logic-devices/${this.idDeviceType}/parameters/${this.parameterId}/options/${optionId}/property`
          );
        } else {
          this.loadDevices();
        }
  ...
  private noErrors(device: ILogicDeviceCommandTypeParameterOption): boolean {
    if (!device.Text) {
