...
@Component({
  selector: 'rom-tariff-export-import-log',
...
export class LogComponent
  ...
  private loadData(): void {
    this.subscription$ = this.loadLogData()
      .pipe(finalize(() => { this.loadingContentPanel = false; this.onLoadEnded.emit(true); }))
      .subscribe(
  ...
  private loadLogData(): Observable<IDatabaseTaskLogRecord | IDatabaseTaskLogRecord[]> {
    return this.exportImportService.getTaskLog(this.data.Id);
  }
}
