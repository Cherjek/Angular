import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';

import { ControlsModule } from '../../../controls/ct.module';
import { SharedModule } from '../../../shared/shared.module';
import { CoreModule } from '../../../core/core.module';

import { SharedHierarchyModule } from '../shared/shared.module';
import { NodeLogicDevicesEditRoutingModule } from './node-logic-devices-edit-routing.module';

import { NodeLogicDevicesEditComponent } from './components/node-logic-devices-edit.component';

@NgModule({
    declarations: [
        NodeLogicDevicesEditComponent
    ],
    imports: [
        CommonModule,
        ControlsModule,
        SharedModule,
        CoreModule,
        
        SharedHierarchyModule,
        NodeLogicDevicesEditRoutingModule
    ]
})
export class NodeLogicDeviceEditModule { }