...
const keyStorageCategoryPosition =
  'Node.OECurrentDataComponent.storageCategoryPosition';

const notFoundClass = 'data-not-found-2';
const filterAttribute = 'data-filter';

@Component({
  selector: 'rom-nc-current-data',
  templateUrl: './current-data.component.html',
  styleUrls: ['./current-data.component.less'],
  providers: [CurrentDataService, DataResultSettingsService, NgbTooltipConfig],
})
export class CurrentDataComponent
  ...
  constructor(
    ...
    this.toolTipconfig.triggers = 'hover focus click';
    this.toolTipconfig.placement = 'bottom';
    currentDataService.typeEntity = TypeEntity.Node;
