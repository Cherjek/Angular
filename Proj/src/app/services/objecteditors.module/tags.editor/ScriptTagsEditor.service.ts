...
@Injectable()
export class ScriptTagsEditorService extends WebService<any> {
    readonly URL = 'script-tags-editor';
    // readonly containers: any = {};

    // cachedTagTypes: TagTypeView[];

    getUnitTags(idUnit: number) {
      return super.get(`unit-tags/${idUnit}`);
    }

    getLogicDeviceTag(idTag: number) {
      return super.get(`${idTag}`);
    }

    getScripts() {
      return super.get('scripts');
    }

    save(tag: any) {
      return super.post(tag);
    }
